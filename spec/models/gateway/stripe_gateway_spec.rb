...
describe Spree::Gateway::StripeGateway do
  ...
  describe 'gateway option parameters (including Stripe Connect)' do
    let(:provider)                    { double('provider') }
    let(:destination)                 { SecureRandom.hex(4) }
    let(:gateway_customer_profile_id) { SecureRandom.hex(4) }
    let(:gateway_payment_profile_id)  { SecureRandom.hex(4) }
    let(:version)                     { SecureRandom.hex(4) }
    let(:app_info)                    { name_with_version = "SpreeGateway/#{SpreeGateway.version} https://spreecommerce.org" }
    let(:order_id)                    { '12345' }
    let(:currency)                    { 'USD' }
    let(:money)                       { 19.99 }
    let(:application_fee)             { 1.99 }
    let(:cc)                          { double('credit_card') }
    let(:gateway_options)             { 
                                        { 
    ...
    let(:options)                     { 
                                        { application: app_info, 
                                          ...
                                          application_fee: application_fee, 
                                          currency: currency, 
                                          description: "Spree Order ID: #{order_id}", 
                                          customer: gateway_customer_profile_id, 
                                          version: version
                                        } 
                                      }
    
    before do
      allow(cc).to receive(:gateway_customer_profile_id).and_return(gateway_customer_profile_id)
      allow(cc).to receive(:gateway_payment_profile_id).and_return(gateway_payment_profile_id)
      subject.stub(:provider).and_return provider
    end

    after do
      subject.purchase(money, cc, gateway_options)
    end

    it 'transforms gateway parameters for specific provider parameters' do
      provider.should_receive(:purchase).with(money, gateway_payment_profile_id, options)
    end
  end
end
